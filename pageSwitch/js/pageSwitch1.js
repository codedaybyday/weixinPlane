	...
	//var pageSwitch = (function(elem,options){
		...
		pageSwitch.prototype = {
			...
			_move:function(){
				var _this = this;
				//alert(_this.index);
				//_this.position = _this.direction?$(_this.section).eq(_this.index).position().top:$(_this.section).eq(_this.index).position().left;
_this.position = _this.direction ? $(window).height()*_this.index : $(window).width()*_this.index;

				console.log(_this.direction)//点击导航上的第一个和第二个是，控制台上的结果还是一样，难道是position函数获取错误
				var transform = _this.direction ?'translate(0px,-'+_this.position+'px)':'translate(-'+_this.position+'px,0px)';
